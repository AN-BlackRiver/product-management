...
@section('content')
    <h2 class="mt-3">Редактирование заказа № {{$order->id}}</h2>
    <form method="POST" id="productForm" class="mt-4" action="{{ route('orders.update', $order->id) }}">
        ...
        <div class="mb-3">
            ...
            <input value="{{old('count') ? old('count') : $order->count}}" type="number" min="1" class="form-control" id="productName" name="count">
            @error('count')
            <p class="text-danger mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input value="{{old('customer_name') ? old('customer_name') : $order->customer_name }}" type="text" class="form-control" id="productName" name="customer_name">
            @error('customer_name')
            <p class="text-danger mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="productDescription" name="description" rows="2">{{old('description') ? old('description') : $order->description}}</textarea>
        </div>
